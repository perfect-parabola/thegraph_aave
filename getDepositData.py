...
# write data file
filename = 'data/ReserveDepositData_{}_{}.csv'.format(
    token_symbol,
    datetime.datetime.now().strftime('%Y.%m.%d_%H:%M:%S')
)
with open(filename, mode='w') as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['datetime', 'amount'])
    for data in json_data.get('data').get('deposits'):
        csv_writer.writerow([
            toString(data.get('timestamp')), data.get('amount')
        ])
